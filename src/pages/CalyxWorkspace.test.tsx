...
describe("CalyxWorkspace conversation lifecycle", () => {
  ...
  it("starts an auto-retry countdown on a network error and retries when triggered manually", async () => {
    ...
    await act(async () => {
      const retryBtn = Array.from(container.querySelectorAll("button")).find((b) => b.textContent?.includes("Retry now"));
      retryBtn?.click();
    });
    await flush(4);
  ...
  it("cancels auto-retry when the user clicks Cancel", async () => {
    vi.useFakeTimers();
