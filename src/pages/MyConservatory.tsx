...
function AddPlant() {
  ...
  if (!ready) return <><ConservatoryReadinessBanner /><div className="rounded-xl border border-amber-500/40 bg-amber-500/5 p-6"><h2 className="text-2xl font-semibold">Plant entry is locked</h2><p className="mt-2 text-sm text-muted-foreground">{readinessError || "Required readiness checks are not yet verified. Collection entry remains safely blocked."}</p><Link className="mt-4 inline-block text-sm font-medium text-primary" to="/conservatory/readiness">Review blocked gates</Link></div></>;
