...
import { Activity, CheckCircle2, Clock3, RefreshCw, ShieldAlert, XCircle } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#06110b] text-[#f5f0e8]">
      ...
      <main className="mx-auto max-w-7xl px-5 pb-16 pt-24 sm:px-6 lg:px-10">
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3 rounded-lg border border-white/10 bg-black/20 px-4 py-3 text-xs text-[#cfc8b8]/75">
          ...
          <span>Source: {ledger?.source_of_truth ?? 'loading'}</span>
          <span>·</span>
        ...
        <section className="mt-8 rounded-xl border border-white/10 bg-[#0b1c11]/85 p-4 sm:p-6">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="text-2xl" style={{ fontFamily: 'Playfair Display, Georgia, serif' }}>Current execution</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[760px] border-collapse text-left text-sm">
              <thead><tr className="border-b border-white/10 font-mono text-[9px] uppercase tracking-[0.12em] text-[#cfc8b8]/55"><th className="px-3 py-3">Status</th><th className="px-3 py-3">Reference</th><th className="px-3 py-3">Work item</th></tr></thead>
              <tbody>{(ledger?.active ?? []).map((item) => <tr key={`${item.ref ?? ''}-${item.title}`} className="border-b border-white/[0.06] last:border-0"><td className="px-3 py-3"><StatusBadge status={item.status} /></td><td className="px-3 py-3 font-mono text-xs text-[#f6dc82]">{item.ref ?? '—'}</td><td className="px-3 py-3 text-[#f5f0e8]/88">{item.title}</td></tr>)}</tbody>
            </table>
          </div>
        </section>
        ...
        <section className="mt-8 rounded-xl border border-emerald-300/15 bg-[#0b1c11]/85 p-4 sm:p-6">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="text-2xl" style={{ fontFamily: 'Playfair Display, Georgia, serif' }}>Verified completion ledger</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[780px] border-collapse text-left text-sm">
              ...
              <tbody>{(ledger?.completed ?? []).map((item) => <tr key={item.id ?? item.title} className="border-b border-white/[0.06] last:border-0"><td className="px-3 py-3 font-mono text-xs text-[#cfc8b8]/65">{item.id ?? '—'}</td><td className="px-3 py-3"><StatusBadge status="complete" /></td><td className="px-3 py-3 font-mono text-xs text-[#f6dc82]">{item.ref ?? '—'}</td><td className="px-3 py-3 text-[#f5f0e8]/88">{item.title}</td></tr>)}</tbody>
            </table>
          </div>
        </section>
