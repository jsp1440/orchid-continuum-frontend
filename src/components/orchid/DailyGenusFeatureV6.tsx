...
import type { GenusEntry } from '@/lib/genusData';
...
function buildEvidenceRows(entry: GenusEntry, liveCount?: number): EvidenceRow[] {
  const speciesNote = liveCount != null && liveCount > 0
    ? `${liveCount} evidence-grounded species cards from the Calyx backend on this page`
    : 'Species-level evidence packets are currently unavailable';
  return [
    { label: 'Taxonomic scope', value: `${entry.family} / ${entry.tribe}` },
    { label: 'Species represented', value: speciesNote },
    {
      label: 'Genus estimate',
      value: `${entry.speciesCount.toLocaleString()} described or accepted species in the working profile`,
    },
    {
      label: 'Provenance state',
      value: 'Public summary; source-linked research view remains available through deeper pages',
    },
  ];
}
...
const MiniCard: React.FC<{ children: React.ReactNode; className?: string }> = ({
  children,
  className = '',
}) => (
  <article className={`rounded-lg border border-[#dfd1ad] bg-[#fffaf0] p-4 ${className}`}>
    {children}
  </article>
);

const DailyGenusFeatureV6: React.FC = () => {
  ...
  const evidenceRows = useMemo(
    () => buildEvidenceRows(entry, speciesCount > 0 ? speciesCount : undefined),
    [entry, speciesCount],
  );
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div className="max-w-2xl">
            ...
            <h3 className="mt-1 font-serif text-3xl leading-tight text-[#24321f]">
              {entry.genus} as a living exhibit
            </h3>
            <p className="mt-2 text-sm leading-6 text-[#5d684c]">
              A public path from name and image into species, ecological partners, place, season,
              conservation, and visible evidence.
            </p>
          </div>
          <Link
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-4">
          {trails.map((trail) => (
            <MiniCard key={trail.label}>
              <p className="font-mono text-[9px] uppercase tracking-[0.2em] text-[#8a6f2d]">
                {trail.label}
              </p>
              <h4 className="mt-1 font-serif text-lg leading-tight text-[#24321f]">
                {trail.title}
              </h4>
              <p className="mt-2 line-clamp-3 text-[12.5px] leading-5 text-[#5d684c]">
                {trail.body}
              </p>
            </MiniCard>
          ))}
      ...
      <SpeciesExhibit genus={entry.genus} onCountChange={setSpeciesCount} />
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-lg border border-[#d9caa8] bg-[#fffaf0]/95 p-5 shadow-[0_10px_24px_rgba(30,40,20,0.06)]">
          ...
          <h3 className="mt-1 font-serif text-3xl leading-tight text-[#24321f]">
            Partners in the life cycle
          </h3>
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            <MiniCard>
              <div className="flex items-center gap-2 text-[#7b6425]">
                <Bug className="h-4 w-4" />
                <span className="font-mono text-[9px] uppercase tracking-[0.18em]">Pollinators</span>
              </div>
              <p className="mt-3 text-sm leading-6 text-[#3a4630]">{entry.ecology.pollinatorGuild}</p>
              <Link
                to="/pollinators"
                className="mt-4 inline-flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.18em] text-[#7b6425] hover:text-[#24321f]"
              >
            ...
            <MiniCard>
              <div className="flex items-center gap-2 text-[#7b6425]">
                ...
                <span className="font-mono text-[9px] uppercase tracking-[0.18em]">Mycorrhizae</span>
              </div>
          ...
          <div className="mt-5 grid gap-3">
            {summaryCards.map((card) => (
              <MiniCard key={card.label}>
                <div className="flex items-center gap-2 text-[#7b6425]">
                  {card.icon}
                  <span className="font-mono text-[9px] uppercase tracking-[0.18em]">{card.label}</span>
                </div>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-lg border border-[#d9caa8] bg-[#f6f0df]/95 p-5 shadow-[0_10px_24px_rgba(30,40,20,0.06)]">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="font-mono text-[9px] uppercase tracking-[0.24em] text-[#8a8062]">
                Map and Atlas Preview
              </p>
              <h3 className="mt-1 font-serif text-3xl leading-tight text-[#24321f]">
                Records, ranges, and caveats
              </h3>
            </div>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-3">
            {['Observations', 'Herbarium and GBIF', 'Interpreted range'].map((label) => (
              <MiniCard key={label}>
                <p className="font-mono text-[9px] uppercase tracking-[0.16em] text-[#8a6f2d]">
                  {label}
                </p>
                <p className="mt-2 text-xs leading-5 text-[#5d684c]">
                  Displayed as evidence layers, not exact boundaries.
                </p>
              </MiniCard>
            ))}
          </div>
          <Link
            to="/atlas"
            className="mt-5 inline-flex items-center gap-2 rounded-lg border border-[#c7b27a] bg-[#fff8e6] px-4 py-2 font-mono text-[11px] uppercase tracking-[0.18em] text-[#5b4b21] hover:bg-[#f8ecc8]"
          >
        ...
        <div className="rounded-lg border border-[#d9caa8] bg-[#f6f0df]/95 p-5 shadow-[0_10px_24px_rgba(30,40,20,0.06)]">
          ...
          <div className="mt-5 grid grid-cols-6 gap-2 sm:grid-cols-12">
            {MONTHS.map((month, index) => {
              ...
              return (
                <div
                  key={month}
                  className={`rounded-lg border px-2 py-3 text-center font-mono text-[10px] uppercase tracking-[0.1em] ${
                    active
                      ? 'border-[#9c7b25] bg-[#d4b34a] text-[#14281c]'
                      : 'border-[#dfd1ad] bg-[#fffaf0] text-[#8a8062]'
                  }`}
                >
                  {month}
                </div>
          ...
          <p className="mt-4 text-xs leading-5 text-[#5d684c]">
            Genus-level phenology is summarized here; species-level timing belongs in the research profile.
          </p>
        </div>
      </section>
      ...
      <section className="rounded-lg border border-[#d9caa8] bg-[#fffaf0]/95 p-5 shadow-[0_10px_24px_rgba(30,40,20,0.06)]">
        <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
          <div>
            ...
            <h3 className="mt-1 font-serif text-3xl leading-tight text-[#24321f]">
              Why {entry.genus} matters
            </h3>
            <p className="mt-3 text-sm leading-6 text-[#5d684c]">
              Conservation context stays paired with visible evidence so the public story remains inviting
              without hiding uncertainty.
            </p>
            <Link
              to="/conservation"
              className="mt-5 inline-flex items-center gap-2 rounded-lg border border-[#c7b27a] bg-[#f8ecc8] px-4 py-2 font-mono text-[11px] uppercase tracking-[0.18em] text-[#5b4b21] hover:bg-[#efdca7]"
            >
              Conservation hub <ShieldCheck className="h-3.5 w-3.5" />
            </Link>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            {evidenceRows.map((row) => (
              <MiniCard key={row.label}>
                <div className="flex items-center gap-2 text-[#7b6425]">
                  ...
                  <span className="font-mono text-[9px] uppercase tracking-[0.18em]">{row.label}</span>
                </div>
            ...
            <MiniCard>
              <div className="flex items-center gap-2 text-[#7b6425]">
                ...
                <span className="font-mono text-[9px] uppercase tracking-[0.18em]">Research view</span>
              </div>
              <p className="mt-2 text-sm leading-6 text-[#3a4630]">
                Taxonomy, literature, occurrence filters, and trait methods continue beyond the homepage.
              </p>
            </MiniCard>
          </div>
        </div>
...
export default DailyGenusFeatureV6;
