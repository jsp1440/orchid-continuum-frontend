...
function scientificName(name: string): React.ReactNode {
  const [genus, ...rest] = name.trim().split(/\s+/);
  return <><span className="not-italic">{genus}</span>{rest.length ? ` ${rest.join(' ')}` : ''}</>;
}
    ...
    <section className="rounded-xl border border-[#d9caa8] bg-[#fffaf0]/95 p-5 shadow-[0_10px_24px_rgba(30,40,20,0.06)]">
      ...
      {loading && (
        <div className="mt-5 flex min-h-[260px] items-center justify-center rounded-lg border border-dashed border-[#c7b27a] bg-[#f6f0df] text-center">
          <div>
            <Camera className="mx-auto h-8 w-8 text-[#8a6f2d]" />
            <p className="mt-3 font-mono text-[10px] uppercase tracking-[0.16em] text-[#6b664f]">Loading verified Orchid Continuum media</p>
          </div>
        </div>
      )}
        ...
        <>
          <figure className="mt-5 overflow-hidden rounded-lg border border-[#d9caa8] bg-[#1a2e1a]">
            ...
            <figcaption className="bg-[#fffaf0] px-4 py-3 text-sm text-[#4c5841]">
              <p className="font-serif text-lg italic text-[#24321f]">{scientificName(hero.scientific_name)}</p>
              <p className="mt-1 text-xs">Source: {hero.source_name}{hero.attribution ? ` · ${hero.attribution}` : ''}</p>
            </figcaption>
          </figure>
            ...
            {gallery.map((item) => (
              <figure key={item.media_id} className="overflow-hidden rounded-lg border border-[#d9caa8] bg-[#fffaf0]">
                ...
                <figcaption className="p-3">
                  <p className="font-serif text-base italic text-[#24321f]">{scientificName(item.scientific_name)}</p>
                  <p className="mt-1 text-[10px] text-[#6b664f]">{item.source_name}</p>
                </figcaption>
              </figure>
            ))}
          </div>
      ...
      {!loading && media.status === 'no_approved_media' && (
        <div className="mt-5 flex min-h-[260px] items-center justify-center rounded-lg border border-dashed border-[#c7b27a] bg-[#f6f0df] p-6 text-center">
          <div className="max-w-md">
            <Leaf className="mx-auto h-9 w-9 text-[#8a6f2d]" />
            <p className="mt-3 font-serif text-xl text-[#24321f]">No verified Orchid Continuum photograph is available yet for {entry.genus}.</p>
            <p className="mt-2 text-sm leading-6 text-[#5d684c]">The site will not substitute an unrelated orchid, a herbarium sheet, or an external fallback image.</p>
          </div>
        </div>
      )}

      {!loading && media.status === 'invalid_genus' && (
        <div className="mt-5 rounded-lg border border-[#d9caa8] bg-[#f6f0df] p-5 text-center text-sm text-[#5d684c]">
          The current Featured Genus could not be resolved by Calyx.
        </div>
      )}

      {!loading && media.status === 'service_error' && (
        <div className="mt-5 rounded-lg border border-[#d9caa8] bg-[#f6f0df] p-5 text-center">
          <Database className="mx-auto h-7 w-7 text-[#8a6f2d]" />
          <p className="mt-2 font-serif text-lg text-[#24321f]">Calyx media service is temporarily unavailable.</p>
          <p className="mt-1 text-sm text-[#5d684c]">No external image fallback is used.</p>
        </div>
      )}

      <div className="mt-5 flex items-center gap-2 text-[10px] text-[#6b664f]">
        <ShieldCheck className="h-3.5 w-3.5 text-[#8a6f2d]" />
        <span>Featured Genus media is resolved by Calyx from Orchid Continuum-linked taxon records.</span>
      </div>
    </section>
  );
};
