...
const StatusBadge: React.FC<{ status?: string }> = ({ status }) => {
  ...
  return (
    <span className="rounded px-2 py-1 font-mono text-[10px] uppercase tracking-[0.12em]" style={{ background: b.bg, color: b.color }}>
      {b.code}
    </span>
  );
};

function speciesCaption(slot: Slot, eco: SpeciesEcology | null, genusDescription: string): string {
  ...
  const distribution = eco?.distribution || slot.place;
  const bits: string[] = [];
  if (habitat || distribution) {
    bits.push(`${slot.species} is shown here as part of the Orchid Continuum genus feature${distribution ? `, linked to records from ${distribution}` : ''}${habitat ? ` and associated with ${habitat.toLowerCase()}` : ''}.`);
  }
...
const DailyGenusFeatureV2: React.FC = () => {
  ...
  const [visibleIndexes, setVisibleIndexes] = useState<number[]>(() => Array.from({ length: 9 }, (_, i) => i));
  ...
  useEffect(() => {
    ...
    fetchGenusSpecies(base.genus, ctrl.signal, SPECIES_LIMIT)
      .then((photos) => {
        if (!ctrl.signal.aborted) setPool(photos);
      })
      .finally(() => {
  ...
  const slots = useMemo<Slot[]>(() => {
    ...
    const withPhotos = Array.from(byName.values()).filter((s) => s.images.length > 0);
    const withoutPhotos = Array.from(byName.values()).filter((s) => s.images.length === 0);
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      setVisibleIndexes((prev) => {
        ...
        const promoted = current[replaceCell % current.length] ?? 0;
        ...
        const nextVisible = current.map((v, i) => (i === replaceCell % current.length ? replacement : v));
  ...
  if (loading && !slots.length) {
    return <div className="rounded-[2rem] border border-[#d9caa8] bg-[#f6f0df]/95 p-8 text-[#3a4630]">Loading Genus of the Day…</div>;
  }
  ...
  const heroScore = bestUrlScore(hero.images, { source: hero.imageSource, license: hero.imageLicense, name: hero.species });
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
        <div className="lg:col-span-2">
          <div className="relative aspect-[4/5] overflow-hidden rounded-2xl border border-[#d7c79c] bg-[#1a2e1a] shadow-inner">
            {hero.images.length ? (
              <FallbackImage urls={hero.images} alt={hero.species} className="h-full w-full object-cover" fallback={<Placeholder label={hero.species} hero />} />
            ) : (
        ...
        <div className="grid grid-cols-3 gap-3 lg:col-span-3">
          {visibleIndexes.map((slotIndex, cell) => {
            ...
            return (
              <button key={`${s.species}-${slotIndex}-${cell}`} onClick={() => setHeroIndex(slotIndex)} className={`group relative aspect-square overflow-hidden rounded-xl border text-left shadow-sm transition ${selected ? 'border-[#8a6f2d] ring-2 ring-[#c9a84c]' : 'border-[#d7c79c] hover:border-[#8a6f2d]'}`}>
                {s.images.length ? (
                  <FallbackImage urls={s.images} alt={s.species} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" fallback={<Placeholder label={s.species} />} />
                ) : (
