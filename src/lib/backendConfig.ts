 ...
 *
 *   BASE: https://orchidcontinuum.onrender.com
 *
...
export const ATLAS_OCCURRENCES_URL =
  'https://orchidcontinuumharvester2.onrender.com/atlas/occurrences';
...
function normalizeBackend(rows: BackendOccurrence[]): OccurrencePoint[] {
  ...
  rows.forEach((r, i) => {
    ...
    out.push({
      ...
      lat,
      lng,
      species: r.species || r.canonical_name || r.scientific_name || 'Orchidaceae',
...
function fungiNode(data: unknown): WebNodeData {
  ...
  const n = rows.length;
  return { count: n, summary: `${n} partnership${n === 1 ? '' : 's'}`, items: names.slice(0, 3), hasData: true };
}
...
function pollinatorNode(data: unknown): WebNodeData {
  ...
  const n = rows.length;
  return { count: n, summary: `${n} linked`, items: guilds.slice(0, 3), hasData: true };
}
