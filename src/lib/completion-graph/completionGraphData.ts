...
const calyxScienceStatusGate: CompletionNode = {
  ...
  evidence: [
    ...
    { kind: 'file', ref: 'src/lib/calyxScience.ts', note: 'fetchCalyxScienceDashboard() now settles all eight /api/science/* calls independently (Promise.allSettled): a failed section reports sectionErrors[section] and its data stays empty/null rather than the whole dashboard failing; only throws when every section is unreachable. The page distinguishes an UNAVAILABLE section from a confirmed-empty one instead of collapsing both to zero.' },
    ...
    { kind: 'test', ref: 'src/lib/calyxScience.test.ts', note: 'Covers all-succeed, single-section-failure (both a list endpoint and the summary/status endpoints), and all-fail paths against the fixed eight-endpoint fetch order.' },
    { kind: 'test', ref: 'src/lib/ownerSessionVerification.test.ts' },
    { kind: 'test', ref: 'src/lib/ownerControlVerification.test.ts' },
  ],
  nextAction: 'Run this dashboard against a live Calyx backend with a real owner session and record the science departments/gaps actually returned (2 of 6 gate categories remain unevaluated: browser/e2e and deployed/operational). Blocked in the autonomous sandbox: outbound network calls to the Calyx backend require interactive approval that is unavailable there, and a real owner access code is an owner-held credential that cannot be generated by an agent.',
