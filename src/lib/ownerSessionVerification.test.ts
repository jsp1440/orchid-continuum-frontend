...
import { mockFetchSequence, rawResponse, rejectingFetch } from './testing/fetchMock';
...
describe('validateOwnerSession — shared owner-session authorization', () => {
  ...
  it('returns token: "cookie" when authenticated is true and owner is present', async () => {
    globalThis.fetch = mockFetchSequence([{ ok: true, body: BASE_VALID_SESSION }]);
  ...
  it('does NOT return token: "cookie" when authenticated true but owner is empty', async () => {
    globalThis.fetch = mockFetchSequence([{ ok: true, body: { ...BASE_VALID_SESSION, owner: '' } }]);
  ...
  it('rejects restore when authenticated: true but owner is missing from response', async () => {
    globalThis.fetch = mockFetchSequence([{ ok: true, body: { ...BASE_VALID_SESSION, owner: null } }]);
  ...
  it('returns unauthenticated session on network failure', async () => {
    globalThis.fetch = rejectingFetch(new Error('Network error'));
  ...
  it('returns unauthenticated session when response body is not a valid object', async () => {
    globalThis.fetch = vi.fn(async () => rawResponse('"not-an-object"')) as typeof globalThis.fetch;
  ...
  it('returns unauthenticated session when backend reports authenticated: false (expired/revoked)', async () => {
    globalThis.fetch = mockFetchSequence([{ ok: true, body: { ...BASE_VALID_SESSION, authenticated: false, reason: 'Session expired' } }]);
  ...
  it('returns full valid session with token on successful restore', async () => {
    globalThis.fetch = mockFetchSequence([{ ok: true, body: { ...BASE_VALID_SESSION, expires_at: 9999999999 } }]);
  ...
  it('privileged token is absent until both authenticated===true AND owner is non-empty', async () => {
    ...
    globalThis.fetch = mockFetchSequence([{ ok: true, body: { ...BASE_VALID_SESSION, authenticated: false } }]);
